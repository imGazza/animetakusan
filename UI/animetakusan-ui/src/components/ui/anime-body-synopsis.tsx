...
const AnimeBodySynopsis = ({ description }: { description: string }) => {
  ...
  return (
    <div className={cn("flex flex-col p-4 pb-0 border bg-muted rounded-xs", !needsCollapse && "pb-4", "animate-in fade-in duration-300")}>
      <div
        ref={contentRef}
        style={{ height }}
        className={cn(
      ...
      >
        ...
        <div dangerouslySetInnerHTML={{ __html: description }} className="text-sm tracking-wide text-primary/90" />
      </div>
      {needsCollapse && (
        <Button
