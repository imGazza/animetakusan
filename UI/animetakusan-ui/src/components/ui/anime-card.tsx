        ...
        <div className="flex relative animate-in cursor-pointer">
          <div ref={triggerRef} className="group flex flex-col w-full max-w-(--sm-image-width) md:max-w-(--md-image-width)">
            <AspectRatio ratio={37 / 53} data-state={imageLoaded ? 'loaded' : 'loading'} className="bg-muted rounded-sm overflow-hidden relative transform-opacity data-[state=loaded]:opacity-100 data-[state=loading]:opacity-50">
              <AnimeCardImage url={anime.coverImage.extraLarge} title={anime.title.english || anime.title.romaji || ""} onImageLoad={() => setImageLoaded(true)} 
              ...
              <div data-state={imageLoaded ? 'loaded' : 'loading'} className={`absolute inset-0 bg-gradient-to-b from-transparent via-black/10 md:via-black/10 to-black/60 md:to-black/60 duration-${animationDuration} ease-in-out data-[state=loaded]:opacity-100 data-[state=loading]:opacity-0`}/>
              ...
              <div data-state={imageLoaded ? 'loaded' : 'loading'} className={`absolute bottom-2 px-1 md:px-2 text-xs md:text-md tracking-wide font-medium leading-tight text-foreground line-clamp-2 duration-${animationDuration} ease-in-out data-[state=loaded]:opacity-100 data-[state=loading]:opacity-0`}>
