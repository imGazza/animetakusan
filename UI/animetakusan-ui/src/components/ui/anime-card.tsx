      ...
      <HoverCardTrigger asChild>
        <div className="flex relative animate-in cursor-pointer">
          <div ref={triggerRef} className="group flex flex-col w-full max-w-(--sm-image-width) md:max-w-(--md-image-width)">
            <AspectRatio ratio={37 / 53} className={`bg-muted rounded-sm overflow-hidden relative transform-opacity ${imageLoaded ? 'opacity-100' : 'opacity-50'}`}>
            {/* // TODO: Gestisci con tag [data] le animazioni */}
              <img
                src={anime.coverImage.extraLarge}
                alt={anime.title.english || anime.title.romaji || ""}
                className={`h-full w-full object-cover lg:group-hover:scale-[1.1] 
                  transition-all duration-${animationDuration} ease-in-out will-change-transform
              ...
              <div className={`absolute inset-0 bg-gradient-to-b 
                from-transparent via-black/10 md:via-black/10 to-black/60 md:to-black/60 duration-${animationDuration} ease-in-out
                ${imageLoaded ? 'opacity-100' : 'opacity-0'}`}/>
              ...
              <div className={`absolute bottom-2 px-1 md:px-2 text-xs md:text-md tracking-wide 
                font-medium leading-tight text-foreground line-clamp-2 duration-${animationDuration} ease-in-out
                ${imageLoaded ? 'opacity-100' : 'opacity-0'}`}>
